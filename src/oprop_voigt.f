...
c       dx = (const*AV)/AD*domain(id)/float(nx-1)
c       where dx = (const*AV)/AD*dz;   therefore dz = domain(id)/float(nx-1)

	      if (AD .ne. 0.0) then 
		 dx = (const*AV)/AD*(domain(id)/float(nx-1))
	      else 
		 dx = (const*AV)*(domain(id)/float(nx-1))
	      endif 

	      dz=domain(id)/float(nx-1)
